...
if (isset($_GET['download']) && $_GET['download'] === 'csv') {
    // 查詢資料
    $query = "SELECT r.material_id, r.name, r.type, r.unit_price, r.stock_quantity, r.last_update, s.name AS supplier_name 
            ...
            <?php if (count($materials) > 0): ?>
                <?php foreach ($materials as $material): ?>
                    <tr>
                        ...
                        <td><?= htmlspecialchars(number_format($material['unit_price'], 2)) ?></td>
                        <td><?= htmlspecialchars($material['stock_quantity']) ?></td>
                        <td><?= htmlspecialchars($material['last_update']) ?></td>
                        <td><?= htmlspecialchars($material['supplier_name'] ?? '無資料') ?></td>
                    </tr>
                <?php endforeach; ?>
            <?php else: ?>
                <tr>
